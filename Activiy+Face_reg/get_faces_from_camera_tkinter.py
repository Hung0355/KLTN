...
class Face_Register:
    def __init__(self):
        ...
        self.input_uid = tk.Entry(self.frame_right_info)
        self.input_uid_char = ""
    ...
    def GUI_get_input_name(self):
        self.input_uid_char = self.input_uid.get()
    ...
    def GUI_info(self):
        ...
        # Step 2: Input uid and create folders for face
        tk.Label(self.frame_right_info,
                 font=self.font_step_title,
                 text="Step 2: Input name").grid(row=7, column=0, columnspan=2, sticky=tk.W, padx=5, pady=20)

        tk.Label(self.frame_right_info, text="UID: ").grid(row=8, column=0, sticky=tk.W, padx=5, pady=0)
        self.input_uid.grid(row=8, column=1, sticky=tk.W, padx=0, pady=2)
        ...
        self.input_class.grid(row=10, column=1, sticky=tk.W, padx=0, pady=2)
    ...
    def addStudent(self):
        ...
        self.cursor.execute(selectQuery, (self.input_uid_char,self.input_class_char))
        result = self.cursor.fetchone()
        if result:
            updateQuery = f"UPDATE Students SET Name = %s WHERE UID = %s AND ClassCode = %s"
            self.cursor.execute(updateQuery, (self.input_name_char,self.input_uid_char,self.input_class_char))
        else:
            insertQuery = f"INSERT INTO Students (UID, ClassCode,Name) VALUES (%s, %s,%s)"
            self.cursor.execute(insertQuery, (self.input_uid_char,self.input_class_char,self.input_name_char))
