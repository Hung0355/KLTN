...
#path_images_from_camera = "train_dataset/pack4/"
...
def main():
    ...
    # Read people's name in file csv
    existing_names = set()
    ...
    with open("data/features_all.csv", "a", newline="") as csvfile:
        ...
        for person in person_list:
            ...
            # Check people's name is already in file csv
            if person_name in existing_names:
                logging.info("Person %s already exists in the CSV file. Skipping...", person_name)
                continue
            ...
            if features_mean_personX is not None:
                ...
                logging.info("Added features for person %s to CSV file.", person_name)
            else:
                logging.warning("No valid face descriptors found for person %s.", person_name)
            logging.info('\n')
